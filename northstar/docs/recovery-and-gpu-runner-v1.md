...
The [combined fixture](gpu-proof-to-settlement-v1.md) now follows a GPU-resolved checkpoint through data settlement, batch replay and snapshot-fenced mid-settlement SIGKILL recovery. It uses explicit genesis delegation fixtures and a surviving test driver, not fresh delegation or automatic service recovery. Broader partial-upload interruption and resolver mutation coverage, portable GPU provisioning, arbitrary-crash L1 durability and independent acceptance remain open. The production verifier remains disabled by default.

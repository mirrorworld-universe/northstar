...
impl<FG: ForkGraph> ProgramCache<FG> {
    ...
    pub fn extract(
        ...
        // Sonic: Northstar tests and isolated ER root construction can use standalone
        // banks before a BankForks weak reference is installed. In that case, keep the
        // pre-fork-graph behavior and treat cache entries as visible on this branch.
        let fork_graph = self
            .fork_graph
            .as_ref()
            .and_then(|fork_graph| fork_graph.upgrade());
        let locked_fork_graph = fork_graph
            .as_ref()
